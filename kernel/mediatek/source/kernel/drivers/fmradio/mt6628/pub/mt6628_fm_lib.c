 ...
 *
 * mt6628 FM Radio Driver -- chip related functions lib
 *
...
#if 0
static fm_s32 mt6628_writeFA(fm_u16 *buff, fm_u8 fa)
{
    fm_u8 i = 0;
...
static fm_s32 mt6628_Mute(fm_bool mute)
{
    ...
    if (mute == 1) {
        mt6628_write(FM_MAIN_CTRL, (dataRead&0xFFDF) | 0x0020);
    } else {
        mt6628_write(FM_MAIN_CTRL, (dataRead&0xFFDF));
    }
...
#if 0
static fm_s32 mt6628_check_dsp(void)
{
    fm_u8 reg1[] = {0x60, 0x61, 0x62};
    fm_u8 reg2[] = {0x1E, 0x1F, 0x20, 0x21, 0x22, 0x23, 0x24, 0x25};
    fm_u16 tmp_reg = 0;
    fm_s32 i;

    for (i = 0; i < sizeof(reg1)/sizeof(reg1[0]); i++) {
        mt6628_read(reg1[i], &tmp_reg);
        WCN_DBG(FM_NTC | CHIP, "DSP check 0x%02x = 0x%04x\n", reg1[i], tmp_reg);
    }

    mt6628_set_bits(0x60, 0x0000, 0xFFF7);
    for (i = 0; i < sizeof(reg2)/sizeof(reg2[0]); i++) {
        mt6628_read(reg2[i], &tmp_reg);
        WCN_DBG(FM_NTC | CHIP, "DSP check 0x%02x = 0x%04x\n", reg2[i], tmp_reg);
    }
...
#define PATCH_BUF_SIZE 4096*6
...
#ifdef MT6628_FM_FPGA
    FM_LOCK(cmd_buf_lock);
    pkt_size = mt6628_pwrup_fpga_on(cmd_buf, TX_BUF_SIZE);
    ret = fm_cmd_tx(cmd_buf, pkt_size, FLAG_EN, SW_RETRY_CNT, EN_TIMEOUT, NULL);
    FM_UNLOCK(cmd_buf_lock);

    if (ret) {
        WCN_DBG(FM_ALT | CHIP, "mt6628_pwrup_fpga_on failed\n");
        return ret;
    }
    ...
    mt6628_read(0x99, &tmp_reg);

    switch (tmp_reg) {
    case 0x0:
        Chip_Version = mt6628_E1;
        path_patch = MT6628_FM_PATCH_PATH;
        path_coeff = MT6628_FM_COEFF_PATH;
        path_hwcoeff = MT6628_FM_HWCOEFF_PATH;
        WCN_DBG(FM_NTC | CHIP, "chip_version: mt6628_E1\n");
        break;
    case 0x8A01:
    default:
        Chip_Version = mt6628_E2;
        path_patch = MT6628_FM_PATCH_PATH;
        path_coeff = MT6628_FM_COEFF_PATH;
        path_hwcoeff = MT6628_FM_HWCOEFF_PATH;
        WCN_DBG(FM_NTC | CHIP, "chip_version: mt6628_E2\n");
        break;
    }
...
#ifdef MT6628_FM_FPGA
    patch_len = fm_file_read(MT6628_FM_ROM_PATH, dsp_buf, PATCH_BUF_SIZE, 0);
    mt6628_DspPatch((const fm_u8*)dsp_buf, patch_len, IMG_ROM);
#endif
...
#ifdef MT6628_FM_USE_6626EVB
    patch_len = fm_file_read(MT6628_FM_HWCOEFF_PATH, dsp_buf, PATCH_BUF_SIZE, 0);
    mt6628_DspPatch((const fm_u8*)dsp_buf, patch_len, IMG_HW_COEFFICIENT);
#endif
...
#ifdef MT6628_FM_FPGA
    mt6628_write(0x6B, 0x2100);
    mt6628_write(0x60, 0x0007);
    mt6628_write(0xA0, 0xD0B2);
#endif
...
static fm_bool mt6628_SetFreq(fm_u16 freq)
{
...
#ifdef MT6628_FM_USE_6626EVB
    //caculate and set FA
    mt6628_read(FM_CHANNEL_SET, &dataRead);

    switch (Chip_Version) {
    case mt6628_E1:

        if (((fm_u8)((dataRead & 0x1000) >> 12)) ^(channel_parameter[freq - 760] & 0x1)) {
            mt6628_read(0x61, &tempbuff);
            mt6628_write(0x60, 0x330F);
            mt6628_write(0x61, 1);
            mt6628_write(0x6e, 0x0);
            mt6628_write(0x6e, 0x0);
            mt6628_write(0x6e, 0x0);
            mt6628_write(0x6e, 0x0);
            mt6628_write(0x60, 0x0);
            mt6628_write(0x60, 0x4000);
            mt6628_write(0x60, 0x0);
            mt6628_write(0x60, 0x3000);
            mt6628_write(0x60, 0x3001);
            Delayms(3);
            mt6628_write(0x60, 0x3003);
            mt6628_write(0x60, 0x3007);
            mt6628_write(0x60, 0x300f);
            mt6628_write(0x61, tempbuff | 0x3);
            mt6628_write(0x61, tempbuff | 0x2);
            mt6628_write(0x6A, 0x20);
            mt6628_write(0x6B, 0x20);
            Delayms(200);
        }
    ...
    mt6628_writeFA(&dataRead, (channel_parameter[freq - 760]));
    mt6628_write(FM_CHANNEL_SET, (dataRead&0xFC00));
#endif
...
static fm_s32 mt6628_CQI_Get(fm_s8 *buf, fm_s32 buf_len)
{
    ...
    if (!ret && res) {
        ...
        for (i = 0; i < (sizeof(res->cqi) / sizeof(struct mt6628_fm_cqi)); i++) {
            pcqi[i].ch = (pmt6628_cqi[i].ch / 2) + 640;
...
static fm_s32 mt6628_ScanStop(void)
{
    ...
    mt6628_RampDown();
    return mt6628_SetFreq(fm_cb_op->cur_freq_get());
}
