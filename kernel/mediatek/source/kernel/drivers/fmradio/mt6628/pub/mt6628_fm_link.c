...
extern fm_s32 fm_print_cmd_fifo(void)
{
    ...
    while (fm_false == FM_TRACE_EMPTY(cmd_fifo)) {
        ...
        WCN_DBG(FM_ALT | LINK, "trace, type %d, op %d, len %d, %d\n", trace.type, trace.opcode, trace.len, jiffies_to_msecs(abs(trace.time)));
        i = 0;
        while ((trace.len > 0) && (i < trace.len) && (i < FM_TRACE_PKT_SIZE)) {
