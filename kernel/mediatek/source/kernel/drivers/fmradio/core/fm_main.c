...
fm_s32 fm_seek(struct fm *fm, struct fm_seek_parm *parm)
{
    ...
    // seek successfully
    if (true == fm_low_ops.bi.seek(fm->min_freq, fm->max_freq, &(parm->freq), seekdir, space)) {
        parm->err = FM_SUCCESS;
    } else {
...
fm_s32  fm_scan(struct fm *fm, struct fm_scan_parm *parm)
{
    ...
    if (parm->band == FM_BAND_UE) {
        ...
        WCN_DBG(FM_ALT | MAIN, "fm_scan band:%d out of range\n", parm->band);
        parm->err = FM_EPARM;
        ret = -EPERM;
        goto out;
    }
