...
DSI_STATUS DSI_Init(BOOL isDsiPoweredOn)
{
    ...
    if (request_irq(MT6577_DSI_IRQ_ID,
...
void DSI_PHY_TIMCONFIG(LCM_PARAMS *lcm_params)
{
...
#if defined(NT35510_TRULY) ||defined(NT35510_CHUANGSHI)
        
	// +3 is recommended from designer becauase of HW latency
	timcon0.HS_TRAIL	= 20;//20;//((hs_trail_m > hs_trail_n) ? hs_trail_m : hs_trail_n) + 3; from 20
	timcon0.HS_ZERO 	= NS_TO_CYCLE((115 + 6 * ui), cycle_time);
	timcon0.HS_PRPR 	= NS_TO_CYCLE((50 + 4 * ui), cycle_time);
	// HS_PRPR can't be 1.
	if (timcon0.HS_PRPR < 2)
		timcon0.HS_PRPR = 2;
	timcon0.LPX 		= NS_TO_CYCLE(200, cycle_time);
	...
	timcon2.CLK_TRAIL 	= NS_TO_CYCLE(70, cycle_time);
	// CLK_TRAIL can't be 1.
	if (timcon2.CLK_TRAIL < 2)
		timcon2.CLK_TRAIL = 2;
	timcon2.CLK_ZERO 	= NS_TO_CYCLE((300), cycle_time);
	timcon2.LPX_WAIT 	= 1;
	timcon2.CONT_DET 	= 0;

	timcon3.CLK_HS_PRPR	= NS_TO_CYCLE((38 + 95) / 2, cycle_time);
