 ...
 * Description:
 * ------------
 *   This Module defines functions of 6577 fuel gauge
 *
 * Author:
 * -------
 * -------
...
#else
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC] MT6577 FGADC driver probe!! \n" );
...
{
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC] MT6577 FGADC driver remove!! \n" );
...
{
	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "[FGADC] MT6577 FGADC driver shutdown!! \n" );		
}
...
static int __init mt6577_fgadc_init(void)
{
	...
	if (ret) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "****[mt6577_fgadc_driver] Unable to device register(%d)\n", ret);
		return ret;
	}
	
	ret = platform_driver_register(&mt6577_fgadc_driver);
	if (ret) {
		xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "****[mt6577_fgadc_driver] Unable to register driver (%d)\n", ret);
		return ret;
	}

	xlog_printk(ANDROID_LOG_INFO, "Power/Battery", "****[mt6577_fgadc_driver] Initialization : DONE \n");
