...
static DEFINE_SPINLOCK(g_gpt_sched_lock);
static unsigned long g_gpt_flags;
static unsigned long g_gpt_sched_flags;
...
void GPT_SetOpMode(GPT_NUM numGPT, GPT_CON_MODE mode)
{
    ...
    if(mode == GPT_FREE_RUN)
    {
    	clk = DRV_Reg32(g_gpt_addr_clk[numGPT]);
    	clk |= (1<<5); // enable REN Bit 5 for GPT count error
    	DRV_WriteReg32(g_gpt_addr_clk[numGPT], clk);
    }
#endif
    spin_unlock_irqrestore(&g_gpt_lock, g_gpt_flags);
}
...
static void mtk_gpt_debug_function(GPT_NUM numGPT)
{
	...
	last_value = DRV_Reg32(GPT2_COUNT);
	value = DRV_Reg32(GPT2_COUNT);
	if(last_value > value)
	{
		if((last_value - value)< 0x0fffffff) /* not count register overflow, */
		{
			printk("warning: [GPT] count error:Last:0x%x, current:0x%x, clk reg:0x%x\n", 
			last_value, value, DRV_Reg32(g_gpt_addr_clk[numGPT]));
			//DRV_WriteReg32(0, 0x000f);//trigger KE
		}
	}
	else
	{
		if((value - last_value) > 0xff000000)
		{
			printk("warning: [GPT] count error:Last:0x%x, current:0x%x, clk reg:0x%x\n", 
			last_value, value, DRV_Reg32(g_gpt_addr_clk[numGPT]));
			//DRV_WriteReg32(0, 0x000f);//trigger KE
...
unsigned long long sched_clock(void)
{
 //   unsigned long long v = cnt32_to_63(DRV_Reg32(GPT2_COUNT));
     ...
     spin_lock_irqsave(&g_gpt_sched_lock, g_gpt_sched_flags);
    ...
    v *= 1000 << 1;
    do_div(v, 13 << 1);
    mtk_gpt_debug_function(GPT2);
    spin_unlock_irqrestore(&g_gpt_sched_lock, g_gpt_sched_flags);
    return v;
}
...
unsigned long long sched_clock(void)
{
    ...
    spin_lock_irqsave(&g_gpt_sched_lock, g_gpt_sched_flags);
    v = cnt32_to_63(DRV_Reg32(GPT2_COUNT));
...
static void mtk_gpt_init(void)
{
...
#ifdef ENABLE_REN_ECO
    DRV_WriteReg32(GPT2_CLK, 0x0000|(1<<5));       /* enable REN2 5 Bit GPT count is not correct, 13MHz */
#else
    ...
    printk("GPT1_COMPARE = %d, HZ = %d, GPT2 Clk reg:0x%x\n", DRV_Reg32(GPT1_COMPARE), HZ, DRV_Reg32(GPT2_CLK));
}
