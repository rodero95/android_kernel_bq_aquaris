...
makedefs="V=0"
nThreads=`nproc`
doublenThreads=$[nThreads*2]
makejobs="-j$doublenThreads"
echo "Number of available threads: $makejobs"
curdir=`pwd`
...
mkbootimg="../mediatek/build/tools/mkbootimg"
kernel_zimg="arch/arm/boot/zImage"
ram_disk="../mediatek/custom/$MTK_PROJECT/root/ramdisk.cpio.gz"

echo "**** Generating boot image ****"

if [ ! -x ${mkimg} ]; then chmod a+x ${mkimg}; fi
if [ ! -x ${mkbootimg} ]; then chmod a+x ${mkbootimg}; fi

${mkimg} ${kernel_zimg} KERNEL > kernelFile

${mkbootimg} --kernel kernelFile --ramdisk ${ram_disk} -o ./boot.img

echo "**** Process finished! ****"
echo "boot.img file is located in kernel folder root"
