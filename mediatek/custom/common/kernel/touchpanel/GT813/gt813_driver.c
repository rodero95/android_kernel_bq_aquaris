...
extern int i2c_write_dummy( struct i2c_client *client, u16 addr );
...
{             
    ...
    //hwPowerOn(MT65XX_POWER_LDO_VGP, VOL_1800, "TP");  
#endif    
    msleep(10);  ///zhaoshaopeng add 20120825
    // set deep sleep off
    mt_set_gpio_mode(GPIO_CTP_RST_PIN, GPIO_CTP_RST_PIN_M_GPIO);
    mt_set_gpio_dir(GPIO_CTP_RST_PIN, GPIO_DIR_OUT);
    mt_set_gpio_out(GPIO_CTP_RST_PIN, GPIO_OUT_ONE);  
...
//zhaoshaopeng start
#ifdef TPD_HAVE_BUTTON
struct gt813_touchkey_info
{
	unsigned short y[5];
	unsigned short x[5];
	unsigned int size[5];
};
static int gt813_point_num = 0;
//zhaoshaopeng end
static int touch_event_handler(void *unused)
{
    static struct gt813_touchkey_info key_info;
