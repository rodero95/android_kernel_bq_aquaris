...
#endif
#ifdef MT6577
//#include "tpd_custom_gt818b_nanbo.h"
...
static int tpd_keys_local_nanbo[TPD_KEY_COUNT] = TPD_KEYS_NANBO;
static int tpd_keys_dim_local_nanbo[TPD_KEY_COUNT][4] = TPD_KEYS_DIM_NANBO;
static int tpd_keys_local_chaosheng[TPD_KEY_COUNT] = TPD_KEYS_CHAOSHENG;
static int tpd_keys_dim_local_chaosheng[TPD_KEY_COUNT][4] = TPD_KEYS_DIM_CHAOSHENG;
...
reset_proc:   
  ...
  //zhaoshaopeng 20120618 add for v920 nanbo/chaosheng 
	i2c_write_dummy( client, TPD_HANDSHAKING_START_REG );
...
//zhaoshaopeng add end	
    ...
    init_wr_node(client);
#endif  
//zhaoshaopeng 20120618 add for v920 nanbo/chaosheng anbo=0, chaosheng =2
    if(gt818_id_data == 0)////nanbo
    {
        printk("\r\n zhaoshaopeng nanbo config data \r\n");
    if ( tpd_info.version_1 < 0x7A ) // Chip version B    or Version C 
    {
		...
		TPD_DMESG(TPD_DEVICE " B version: 0x4B~0x59, C version 0x5A~0x79\n" );
            cfg_data = cfg_nanbo_version_b;
            cfg_data_with_charger = cfg_nanbo_with_charger_version_b;
    }
    else if ( tpd_info.version_1 < 0xA0 ) 
    {
            ...
            cfg_data = cfg_nanbo_version_d;
            cfg_data_with_charger = cfg_nanbo_with_charger_version_d;
        }
        else
        {
    ...
    {
        printk("\r\n zhaoshaopeng chaosheng config data \r\n");
    if ( tpd_info.version_1 < 0x7A ) // Chip version B    or Version C 
    {
           
        TPD_DMESG(TPD_DEVICE " read version %02X , use B version config\n", tpd_info.version_1 );
		TPD_DMESG(TPD_DEVICE " B version: 0x4B~0x59, C version 0x5A~0x79\n" );
            cfg_data = cfg_chaosheng_version_b;
            cfg_data_with_charger = cfg_chaosheng_with_charger_version_b;
    }
    else if ( tpd_info.version_1 < 0xA0 ) 
    {
        ...
        TPD_DMESG(TPD_DEVICE " read version %02X, use D version config\n", tpd_info.version_1 );
		TPD_DMESG(TPD_DEVICE "  D version: 0x7A~0x99, E version 0x9A~0xB9\n" );
            cfg_data = cfg_chaosheng_version_d;
            cfg_data_with_charger = cfg_chaosheng_with_charger_version_d;
    }
    else
    {
        TPD_DMESG(TPD_DEVICE " unknow Chip version %02X ,use B version config\n", tpd_info.version_1 );
...
/*
       ...
       {                   //we use tianma here because there is no defference
                      	if(virtual_key == 0x08)
				{
				     cinfo.x[0] = tpd_keys_dim_local_nanbo[0][0];
				     cinfo.y[0] = tpd_keys_dim_local_nanbo[0][1];
				     gt818b_point_num = 1;
				}
				else if(virtual_key == 0x04)
				{
				     cinfo.x[0] = tpd_keys_dim_local_nanbo[1][0];
				     cinfo.y[0] = tpd_keys_dim_local_nanbo[1][1];
				     gt818b_point_num = 1;
				}
				else if(virtual_key == 0x02)
				{
				     cinfo.x[0] = tpd_keys_dim_local_nanbo[2][0];
				     cinfo.y[0] = tpd_keys_dim_local_nanbo[2][1];
				     gt818b_point_num = 1;
				}
				else if(virtual_key == 0x01)
				{
				     cinfo.x[0] = tpd_keys_dim_local_nanbo[3][0];
				     cinfo.y[0] = tpd_keys_dim_local_nanbo[3][1];
				     gt818b_point_num = 1;
				}
...
static int tpd_local_init(void) 
{
    ...
    tpd_button_setting(TPD_KEY_COUNT, tpd_keys_local, tpd_keys_dim_local);// initialize tpd button data
       ...
       if(gt818_id_data == 0)//nanbo
       {
           tpd_button_setting(TPD_KEY_COUNT, tpd_keys_local_nanbo, tpd_keys_dim_local_nanbo);// initialize tpd button data
           printk("\r\n this is nanbo_gt818 \r\n");
       }
	else
       {
           tpd_button_setting(TPD_KEY_COUNT, tpd_keys_local_chaosheng, tpd_keys_dim_local_chaosheng);// initialize tpd button data
           printk("\r\n this is chaosheng_gt818 \r\n");	   
	}
#endif  
