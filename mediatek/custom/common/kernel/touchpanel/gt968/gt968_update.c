...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
    ...
    GTP_DEBUG("IC Product id:%c%c%c%c", update_msg.ic_fw_msg.pid[0], update_msg.ic_fw_msg.pid[1],
                                        update_msg.ic_fw_msg.pid[2], update_msg.ic_fw_msg.pid[3]);
...
static u8 gup_enter_upadte_judge(st_fw_head *fw_head)
{
    ...
    GTP_DEBUG("FILE PID:%c%c%c%c", fw_head->pid[0], fw_head->pid[1], fw_head->pid[2], fw_head->pid[3]);
    
    u16_tmp = fw_head->vid;
    fw_head->vid = (u16)(u16_tmp>>8) + (u16)(u16_tmp<<8);
    
    GTP_DEBUG("FILE VID:0x%04x", fw_head->vid);
    GTP_DEBUG("IC HARDWARE INFO:%02x%02x%02x%02x", update_msg.ic_fw_msg.hw_info[0], update_msg.ic_fw_msg.hw_info[1],
                                                   update_msg.ic_fw_msg.hw_info[2], update_msg.ic_fw_msg.hw_info[3]);
    GTP_DEBUG("IC PID:%c%c%c%c", update_msg.ic_fw_msg.pid[0], update_msg.ic_fw_msg.pid[1], update_msg.ic_fw_msg.pid[2], update_msg.ic_fw_msg.pid[3]);
    GTP_DEBUG("IC VID:0x%04x", update_msg.ic_fw_msg.vid);
    ...
    if ( !memcmp(fw_head->hw_info, update_msg.ic_fw_msg.hw_info, sizeof(update_msg.ic_fw_msg.hw_info)))
    {
        ...
        if (( !memcmp(fw_head->pid, update_msg.ic_fw_msg.pid, sizeof(update_msg.ic_fw_msg.pid)-1))||
...
u8 gup_init_update_proc(struct i2c_client *client)
{
    ...
    GTP_INFO("Ready to run update thread.%d,%d",client,guitar_client);
