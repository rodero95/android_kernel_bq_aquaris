...
#include "tpd_custom_gt813.h" //tpd_custom_GT813.h
#include "tpd.h"
#include <cust_eint.h>
...
static int tpd_calmat_local[8]     				= TPD_CALIBRATION_MATRIX_ROTATION_90;
static int tpd_def_calmat_local[8] 				= TPD_CALIBRATION_MATRIX_ROTATION_90;
static int tpd_def_calmat_local_rotation_0[8]   = TPD_CALIBRATION_MATRIX_ROTATION_0;
...
{             
    ...
    *(volatile u16 *) TPD_RESET_PIN_ADDR = temp;
    ...
    TPD_DMESG("MediaTek touch panel i2c probe - gt813 \n");
    
    //Power on
    hwPowerOn(MT65XX_POWER_LDO_VGP, VOL_2800, "TP");  
    TPD_DMESG("MediaTek touch panel sets VGP hwPowerOn!!!\n");
    ...
    if ( err )
    {
        TPD_DMESG(" fail to get tpd info %d\n", err );
        return err;
    }
    else
    {
        TPD_DMESG( "Goodix TouchScreen Version:%s\n", (char *)&version[0]);
    }
#endif    
    //Load the init table
...
int gt813_open ( struct inode *inode, struct file *filp )
{
	TPD_DEBUG("gt813 open!\n");
	return 0;
}

int gt813_close ( struct inode *inode, struct file *filp )
{
	TPD_DEBUG("gt813 close!\n");
	return 0;
}

int gt813_unlocked_ioctl ( struct inode *inode, struct file *filp, unsigned int cmd, unsigned long arg)
{
	TPD_DEBUG("gt813 ioctl!\n");

	switch ( cmd )
	{
		case TPD_IOCTL_ENABLE_I2C:
			TPD_DEBUG("gt813 TPD_IOCTL_ENABLE_I2C!\n");
			mt65xx_eint_unmask(CUST_EINT_TOUCH_PANEL_NUM);
			break;
			
		case TPD_IOCTL_DISABLE_I2C:
			TPD_DEBUG("gt813 TPD_IOCTL_DISABLE_I2C!\n");
			mt65xx_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
			break;
			
		default:
			TPD_DEBUG("gt813 command does not exist!\n");
			break;
	}
...
struct file_operations gt813_fops = {
	.owner = THIS_MODULE,
	.open = gt813_open,
	.release = gt813_close,
	.unlocked_ioctl = gt813_unlocked_ioctl,
};
...
void tpd_key_up ( u8 keyStatus )
{
	if ( TPD_KEY_MENU == keyStatus )
    {
		...
		TPD_DEBUG("[mtk-tpd] Release key menu!!\n");
    }
	else if ( TPD_KEY_HOME == keyStatus )
    {
        input_report_key( tpd->dev, KEY_HOME, 0 );
		TPD_DEBUG("[mtk-tpd] Release key home!!\n");
    }
	else if ( TPD_KEY_BACK == keyStatus )
    {
        input_report_key( tpd->dev, KEY_BACK, 0 );
		TPD_DEBUG("[mtk-tpd] Release key keyback!!\n");
    }
	//TPD_DEBUG("[mtk-tpd] Call input_mt_sync");
	input_mt_sync( tpd->dev );
}
...
static int touch_event_handler(void *unused) {
    ...
    do{
        ...
        if ( cur_mask )
        {
			...
			finger_num = idx;
			max_finger_id = idx - 1;
            ret = i2c_read_bytes( i2c_client, TPD_POINT_INFO_REG_BASE, buffer, finger_num * TPD_POINT_INFO_LEN );
            if ( ret < 0 )
            {
        ...
        for ( idx = 0, valid_point = 0 ; idx < ( max_finger_id + 1 ) ; idx++ )
        {
            ...
            if ( cur_mask & ( 1 << idx ) )
            {
                ...
                y = ptr[3] + ( ( (int)ptr[2] ) << 8 );
                size = ptr[4];              
                TPD_DEBUG("[mtk-tpd] Original position: [%4d %4d]\n", x, y);
...
int tpd_local_init(void) 
{
	...
	/* register device (/dev/gt813 CTP) */
	retval = misc_register( &tpd_dev );
	TPD_DEBUG("GT813 call misc_register\n");
	if ( 0 != retval )
	{
		TPD_DEBUG("Register gt813 device failed (%d)\n", retval);
		return -1;
	}
	...
	if ( FACTORY_BOOT == get_boot_mode() )
	{
		...
		memcpy(tpd_calmat, tpd_def_calmat_local_rotation_0, 8*4);
	    memcpy(tpd_def_calmat, tpd_def_calmat_local_rotation_0, 8*4);		
	}
	else
	{
...
void tpd_suspend(struct i2c_client *client, pm_message_t message)
{
    ...
    tpd_halt = 1;
    mt65xx_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
    TPD_DEBUG(TPD_DEVICE " tpd_suspend start \n");
    
#ifdef ESD_PROTECT
    cancel_delayed_work_sync(&tpd_esd_check_work); 
#endif
...
{   
    TPD_DEBUG(TPD_DEVICE " tpd_resume start \n");  
