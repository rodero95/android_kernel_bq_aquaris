...
#define STK_OPEN_PROX_ARITHMETIC 1
/******************************************************************************
 * extern functions
*******************************************************************************/
#ifdef MT6516
...
static unsigned short stk31xx_force[] = {0x00, 0x90, I2C_CLIENT_END, I2C_CLIENT_END};
...
extern struct alsps_hw_stk31xx *stk31xx_get_cust_alsps_hw(void);
extern int hwmsen_asensor_add(struct sensor_init_info* obj) ;
...
static int  stk31xx_local_init(void);
static int stk31xx_remove(void);
static int stk31xx_init_flag =0;
static struct sensor_init_info stk31xx_init_info = {			
	.name = "stk31xx",			
	.init = stk31xx_local_init,			
	.uninit =stk31xx_remove,	
};/* Add end */
...
/*----------------------------------------------------------------------------*/
int stk31xx_get_addr(struct alsps_hw_stk31xx *hw, struct stk31xx_i2c_addr *addr)
{
	if(!hw || !addr)
	{
		return -EFAULT;
...
/*----------------------------------------------------------------------------*/
static void stk31xx_power(struct alsps_hw_stk31xx *hw, unsigned int on) 
{
...
//Auto Calibration
#ifdef STK_OPEN_PROX_ARITHMETIC
static int stk3171_init_client_for_cali(void)
{
	int i=0 ,err = 0,j = 0;
	u16 data[22]={0},sum = 0,data_cali=0;
	struct i2c_client *client;
	client = stk31xx_i2c_client;
	struct stk31xx_priv *obj = i2c_get_clientdata(client);
	...
	mdelay(50);
	 ...
	 {
			mdelay(5);//50
			if(err = stk31xx_read_ps(client,&data[i]))
			{
				APS_ERR("stk31xx_read_ps fail: %d\n", i); 
				break;
			}
//			else
//			{
...
//			}
	...
	{
		APS_LOG("%s disable ps correct\n", __FUNCTION__);
	}	 
	 
	for(j = 1;j<=20;j++){
		APS_LOG("%s [data%d]:%d,", __FUNCTION__,j,data[j]);
		sum += data[j];
	}
	if(i == 21)
	{
		data_cali = sum/20;
		APS_LOG("\n%s stk31xx read average ps data = %d\n", __FUNCTION__,data_cali);
	}


	atomic_set(&obj->ps_high_thd_val, data_cali + 15 ); 
	atomic_set(&obj->ps_low_thd_val, data_cali + 10 ); 
	APS_LOG("%s Calibration PS High Threshold = %d, Low Threshold = %d\n", __FUNCTION__,atomic_read(&obj->ps_high_thd_val),atomic_read(&obj->ps_low_thd_val));
...
#ifdef STK_OPEN_PROX_ARITHMETIC
	if (stk3171_init_client_for_cali() == 0)	//auto calibration ps threshold
	{
		APS_LOG("%s stk3171_init_client_for_cali OK\n", __FUNCTION__);
	}
#endif
	/* Add for auto detect feature */	
    stk31xx_init_flag = 0;	
...
{
	struct alsps_hw_stk31xx *hw = stk31xx_get_cust_alsps_hw();
	struct stk31xx_i2c_addr addr;
...
static int __init stk31xx_init(void)
{
	...
	APS_LOG("stk31xx_init...\n");
	hwmsen_asensor_add(&stk31xx_init_info);	
	return 0;
}
/*----------------------------------------------------------------------------*/
