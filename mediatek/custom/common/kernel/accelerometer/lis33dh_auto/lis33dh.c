...
static int LIS33DH_Init(struct i2c_client *client, int reset_cali)
{
	...
	res = LIS33DH_SetBWRate(client, LIS33DH_BW_100HZ);//400 or 100 no other choice
	if(res != LIS33DH_SUCCESS )
	{
		return res;
	}
	...
	switch (command)
	{
		case SENSOR_DELAY:
			...
			else
			{
				...
				if(value <= 5)
				{
					sample_delay = LIS33DH_BW_200HZ;
				}
				else if(value <= 10)
				{
					sample_delay = ~LIS33DH_BW_100HZ;
				}
				else
				{
					sample_delay = ~LIS33DH_BW_50HZ;
				}
				
				err = LIS33DH_SetBWRate(priv->client, sample_delay);
				if(err != LIS33DH_SUCCESS ) //0x2C->BW=100Hz
				{
					GSE_ERR("Set delay parameter error!\n");
				}
...
static int __init lis33dh_init(void)
{
	...
	i2c_register_board_info(0, &i2c_lis33dh, 1);
