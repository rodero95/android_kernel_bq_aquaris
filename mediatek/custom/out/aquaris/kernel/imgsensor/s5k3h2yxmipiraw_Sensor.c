...
void S5K3H2YXMIPISetFlipMirror(kal_int32 imgMirror)
{
    ...
    switch (imgMirror)
    {
        case IMAGE_HV_MIRROR://IMAGE_NORMAL:
        ...
        case IMAGE_H_MIRROR:
            ...
            break;
        case IMAGE_NORMAL://IMAGE_HV_MIRROR:
                                                ...
                                                MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	  ...
	  S5K3H2YXMIPISetFlipMirror(sensor_config_data->SensorImageMirror);	
       ...
       shutter = (shutter * S5K3H2YXMIPI_sensor.pv_line_length)/S5K3H2YXMIPI_sensor.cp_line_length;
	   SENSORDB("[S5K3H2YXMIPI]cp_shutter=%d,cp_length=%d\n",shutter,S5K3H2YXMIPI_sensor.cp_line_length); 
	   #else
	   //calculate cp shutter 
	   shutter =  (S5K3H2YXMIPI_sensor.cp_pclk/100000) * shutter/(S5K3H2YXMIPI_sensor.pv_pclk/100000);
	   shutter = (shutter * S5K3H2YXMIPI_sensor.pv_line_length)/S5K3H2YXMIPI_sensor.cp_line_length;
	   SENSORDB("[S5K3H2YXMIPI]cp_shutter=%d,cp_length=%d\n",shutter,S5K3H2YXMIPI_sensor.cp_line_length);  
	  ...
	  // shutter = shutter *9858/10000;
	    ...
	    //shutter *= 2; //preview binning 

	   	if(shutter < 3)
	    shutter = 3;

		//After call S5K4E5YACaptureSetting(), gain is initialized, so we have to reset preview gain here.
		S5K3H2YXMIPI_SetGain(S5K3H2YXMIPI_sensor.sensorGlobalGain*S5K3H2YXMIPI_sensor.ispBaseGain/S5K3H2YXMIPI_sensor.sensorBaseGain);//set preview gain	
	
		SENSORDB("[S5K3H2YXMIPICapture]cap_shutter =%d ,gain = %d\n",shutter,S5K3H2YXMIPI_sensor.sensorGlobalGain);

		S5K3H2YXMIPI_write_shutter(shutter);
  ...
  //  pSensorInfo->SensorCameraPreviewFrameRate=30;
    ...
    switch (ScenarioId)
    {
		...
		case MSDK_SCENARIO_ID_CAMERA_ZSD:
            ...
            pSensorInfo->SensorPixelClockCount= 3;
            pSensorInfo->SensorDataLatchCount= 2;
//            pSensorInfo->SensorGrabStartX = S5K3H2YXMIPI_IMAGE_SENSOR_PV_STARTX; 
                                                                ...
                                                                UINT8 *pFeaturePara,UINT32 *pFeatureParaLen)
{
    ...
    switch (FeatureId)
    {
        ...
        case SENSOR_FEATURE_GET_ENG_INFO:
            ...
            pSensorEngInfo->SensorOutputDataFormat=SENSOR_OUTPUT_FORMAT_RAW_R;//SENSOR_OUTPUT_FORMAT_RAW_B; zhaoshaopeng 
