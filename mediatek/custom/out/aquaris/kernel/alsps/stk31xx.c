...
//			}
	...
	{
		printk("%s disable ps correct\n", __FUNCTION__);
	}	 

	 for(j = 1;j<=20;j++){
		printk("%s [data%d]:%d,", __FUNCTION__,j,data[j]);
		sum += data[j];
	 }
	 if(i == 21)
	 {
	 ...
	 if(data_cali < 255)
	 {
		atomic_set(&obj->ps_high_thd_val, data_cali + 20 ); 
		atomic_set(&obj->ps_low_thd_val, data_cali + 15 ); 
		printk("%s Calibration PS High Threshold = %d, Low Threshold = %d\n", __FUNCTION__,atomic_read(&obj->ps_high_thd_val),atomic_read(&obj->ps_low_thd_val));
	 ...
	 else
	 {
		...
		atomic_set(&obj->ps_low_thd_val, data_cali - 25 ); 
		printk("%s Calibration PS High Threshold = %d, Low Threshold = %d\n", __FUNCTION__,atomic_read(&obj->ps_high_thd_val),atomic_read(&obj->ps_low_thd_val));
...
/*----------------------------------------------------------------------------*/
static long stk31xx_unlocked_ioctl(struct file *file, unsigned int cmd,
       unsigned long arg)
{
...
//	}
	...
	{
		printk("%s stk3171_init_client_for_cali OK\n", __FUNCTION__);
	}
#endif
	/* Add for auto detect feature */	
    stk31xx_init_flag = 0;	
...
#else	
	APS_ERR("stk31xx_init...\n");
	hwmsen_alsps_sensor_add(&stk31xx_init_info);	
#endif		
	return 0;
}
/*----------------------------------------------------------------------------*/
