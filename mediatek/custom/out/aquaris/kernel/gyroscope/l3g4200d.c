...
static struct i2c_board_info __initdata i2c_l3g4200d={ I2C_BOARD_INFO("L3G4200D", (0xD0>>1))};
		...
		void* buff_out, int size_out, int* actualout)
{
	...
	char buff[L3G4200D_BUFSIZE];	
       printk("\r\n zhaoshaopeng l3g4200d_operate cmd=%d \r\n", command);
	switch (command)
	{
		case SENSOR_DELAY:
		...
		case SENSOR_GET_DATA:
			...
			else
			{
				...
				printk("\r\n zhaoshaopeng test gyro_data->values[0]=%x,gyro_data->values[1]=%x,gyro_data->values[2]=%x \r\n", gyro_data->values[0],gyro_data->values[1],gyro_data->values[2]);
			}
			break;
		default:
...
//       unsigned long arg)
static int l3g4200d_unlocked_ioctl(struct file *file, unsigned int cmd,
       unsigned long arg)
{
...
static struct file_operations l3g4200d_fops = {
	...
	.unlocked_ioctl = l3g4200d_unlocked_ioctl,
};
/*----------------------------------------------------------------------------*/
...
static int l3g4200d_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	int err = 0;
	GYRO_FUN();
       printk("\r\n zhaoshaopeng l3g4200d_i2c_probe \r\n");
